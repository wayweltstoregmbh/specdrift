...
function report(rest) {
  ...
  const strictFail = Boolean(flags.strict)
    && body !== null
    && (body.report_status !== "complete" || driftCount > 0);
  emit(output, flags.json, () => {
    if (body) {
      const display = findings.length > 0 || strictFail ? "fail" : output.status;
      process.stdout.write(
        `${display}: report ${body.report_status}, coverage ${body.coverage_percent}%, ` +
          `${body.totals.missing} missing, ${body.totals.distorted} distorted, ` +
          `${body.totals.waived} waived, ${body.additions.length} addition(s)\n`
      );
      for (const entry of body.drift.missing) {
        process.stdout.write(
          `missing: ${entry.claim_id} ${entry.source_file}:${entry.source_line} — ${entry.source_text}\n`
        );
      }
      for (const entry of body.drift.distorted) {
        process.stdout.write(
          `distorted: ${entry.claim_id} ${entry.source_file}:${entry.source_line} — ` +
            `${entry.source_text}${entry.reason ? ` (${entry.reason})` : ""}\n`
        );
      }
...
function gate(rest) {
  ...
  emit(output, flags.json, () => {
    ...
    if (result.report) {
      for (const entry of result.report.drift.missing) {
        process.stdout.write(
          `missing: ${entry.claim_id} ${entry.source_file}:${entry.source_line} — ${entry.source_text}\n`
        );
      }
      for (const entry of result.report.drift.distorted) {
        process.stdout.write(
          `distorted: ${entry.claim_id} ${entry.source_file}:${entry.source_line} — ${entry.source_text}\n`
        );
      }
    }
...
function claimsList(rest) {
  const { flags, positional } = parseFlags(rest, { "--claims": "value", "--json": "boolean" });
  if (positional.length > 0) throw new Error(`unexpected argument "${positional[0]}"`);
  const cwd = process.cwd();
  const claimsRel = flags.claims || coverage.DEFAULT_CLAIMS;
  const findings = [];
  ...
  if (!store) throw new Error(`${findings[0]} — run "specdrift extract <spec>" first`);
  if (flags.json) {
    const output = { tool: "claims list", status: "pass", claims: store.claims };
    process.stdout.write(`${JSON.stringify(output, null, 2)}\n`);
    return 0;
  }
  for (const claim of store.claims) {
    const text = claim.source_text.length > 64
      ? `${claim.source_text.slice(0, 61)}...`
      : claim.source_text;
    process.stdout.write(
      `${claim.claim_id}  ${claim.claim_type.padEnd(14)} ${claim.strength.padEnd(10)} ` +
        `${claim.source_file}:${claim.source_line_start}  ${text}\n`
    );
  }
  process.stdout.write(`${store.claims.length} claim(s) from ${store.sources.length} source(s)\n`);
  return 0;
}
...
const commands = {
  "extract": {
    ...
    usage: "<spec-file-or-dir> [--out <file>] [--json]",
    run: extract
  },
  "claims list": {
    summary: "List all claims with id, type, strength and source line",
    usage: "[--claims <file>] [--json]",
    run: claimsList
  },
  "status": {
    summary: "Report claim coverage, contract confirmation state and source staleness",
    usage: "[--claims <file>] [--contracts <file>] [--json]",
    run: status
  },
  "verdict record": {
    summary: "Record a per-claim verdict (covered/missing/distorted/out_of_scope/accepted_deviation)",
    usage: "<claim_id> <verdict> [--evidence <file[:line]>] [--reason <text>] " +
      "[--approved-by <name>] [--overwrite] [--json]",
    run: verdictRecord
  },
  "verdict addition": {
    summary: "Record an out-of-scope addition (unrequested change) with evidence",
    usage: "--evidence <file[:line]> --note <text> [--json]",
    run: verdictAddition
  },
  "report": {
    summary: "Build the drift report from claims and verdicts (--strict gates on drift)",
    usage: "[--strict] [--out <file>] [--claims <file>] [--verdicts <file>] [--json]",
    run: report
  },
  "gate": {
    summary: "CI gate: fail on drift, low coverage or uncovered changes; emits a PR comment body",
    usage: "[--fail-on missing|distorted|both] [--min-coverage <pct>] [--max-uncovered <n>] " +
      "[--changed <file,file>] [--out <file>] [--comment-out <file>] [--json]",
    run: gate
  },
  "confirm": {
    summary: "Bind current contract candidates into confirmed contracts (--accept-current)",
    usage: "--accept-current [--claims <file>] [--out <file>] [--json]",
    run: confirm
  },
  "schema check": {
